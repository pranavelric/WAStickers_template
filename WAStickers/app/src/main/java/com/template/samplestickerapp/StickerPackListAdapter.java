...
import com.template.samplestickerapp.main.utils.MySharedPrefrences;
...
public class StickerPackListAdapter extends RecyclerView.Adapter<StickerPackListItemViewHolder> {
    ...
    private StickerPackListActivity activity;

    StickerPackListAdapter(@NonNull List<StickerPack> stickerPacks, @NonNull OnAddButtonClickedListener onAddButtonClickedListener, Context context, StickerPackListActivity activity) {
        this.stickerPacks = stickerPacks;
        this.onAddButtonClickedListener = onAddButtonClickedListener;
        mySharedPrefrences = new MySharedPrefrences(context.getApplicationContext());
        ...
        if (pack.getIsWhitelisted()) {
            ...
            addButton.setOnClickListener(v -> {
                onAddButtonClickedListener.onAddButtonClicked(pack);
                        ...
                        //show rateus Dialog here
                        if (mySharedPrefrences.getSecondCount() > 1) {

                            Log.d("RRR", "setAddButtonAppearance: "+activity.getPackageName());
                            SmartRate.Rate(activity
                                    , "Rate Us"
                            ...
                            mySharedPrefrences.setIsSecondDownloadCount(0);
                        } else {
                            mySharedPrefrences.setIsSecondDownloadCount(mySharedPrefrences.getSecondCount() + 1);
                        }
